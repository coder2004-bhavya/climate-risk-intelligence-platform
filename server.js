...
app.post('/api/predict', authenticate, async (req, res) => {
    try {
        // Forward to Flask Server running on port 8000
        const pythonResponse = await axios.post('http://127.0.0.1:8000/api/predict', req.body);
...
app.post('/api/chat', async (req, res) => {
    try {
        const pythonResponse = await axios.post('http://127.0.0.1:8000/api/chat', req.body);
        res.json(pythonResponse.data);
    } catch (error) {
